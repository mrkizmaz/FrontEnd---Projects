import React from 'react'
import { useDispatch, useSelector } from 'react-redux'
import Loader from '../../components/loader';
import Error from '../../components/error';
import { baseImgUrl } from '../../constant';
import { Link } from 'react-router-dom';
import { GoBookmarkSlashFill } from 'react-icons/go';
import { MdBookmarkRemove } from 'react-icons/md';
import { toggleList } from '../../redux/actions';

const Favorites = () => {
    // storea abone olma
    const { isLoading, error, list } = useSelector((store) => store);
    // console.log(state);

    const dispatch = useDispatch();

    const handleDelete = (movie) => {
        dispatch(toggleList(movie, false));
    }



    if (isLoading) return <Loader />

    if (error) return <Error info={error} />


    return (
        <div>

            <h1 className='text-2xl md:text-3xl font-semibold'>Izleme Listesi</h1>

            <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-5 my-10'>
                {list.map((movie, key) => (
                    <div key={key} className='relative'>
                        <button onClick={() => handleDelete(movie)}
                            className='absolute top-3 end-3 bg-blue-500 p-2 rounded hover:bg-blue-600 transition z-10 shadow'>
                            <MdBookmarkRemove className='text-2xl' />
                        </button>
                        <Link to={`/movie/${movie.id}`}>
                            <img className='rounded'
                                src={baseImgUrl + movie.poster_path} alt="" />
                        </Link>
                        <h1 className='text-center text-xl font-semibold mt-3'>{movie.title}</h1>
                    </div>
                ))}
            </div>

        </div>
    )
}

export default Favorites